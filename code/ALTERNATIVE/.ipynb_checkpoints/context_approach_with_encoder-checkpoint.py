...
from preprocessing import get_words_with_freq_from_text, load_encoder_decoder
    ...
    def __init__(self, load_bi_grams_path=None, load_tri_grams_path=None, create_grams_path=None, encoder_decoder_path=None,
        ...
        try:
            if load_bi_grams_path:
                ...
                self.bi_grams, self.idx2word, self.word2idx = get_bigrams_from_sentences_given_in_lines(create_grams_path, 2, threshold)
            else:
                self.bi_grams = None
            ...
            if load_tri_grams_path:
                ...
                self.tri_grams, self.idx2word, self.word2idx = get_trigrams_from_sentences_given_in_lines(create_grams_path, 3, threshold)
            else:
                self.tri_grams = None
        except:
            print("wrong path")
            
        if encoder_decoder_path:
             self.word2idx, self.idx2word = load_encoder_decoder(encoder_decoder_path)
